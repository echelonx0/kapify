...
export class FunderDocumentAnalysisService {
  ...
  private async performAnalysis(file: File): Promise<DocumentAnalysisResult> {
    ...
    try {
      ...
      if (extractedText.length > 100000) {
        ...
        throw new Error(
          `Document too large: ${charCount} characters (limit: 100,000). Please use a shorter document.`
        );
      }
      const analysisResult = await this.callAnalysisEdgeFunction(
        extractedText,
        file.name
      );
    ...
    try {
      ...
      try {
        ...
        if (this.isValidText(cleanText)) {
          console.log(
            'Gemini AI extraction successful, length:',
            cleanText.length
          );
          return cleanText;
        }
      ...
      try {
        ...
        if (this.isValidText(cleanText)) {
          console.log(
            'PDF.js extraction successful, length:',
            cleanText.length
          );
          return cleanText;
        }
      ...
      throw new Error(
        'Unable to extract readable text. This PDF may be image-based, corrupted, or unsupported.'
      );
    } catch (error) {
  ...
  private async callAnalysisEdgeFunction(
    ...
    if (!content || content.trim().length < 100) {
      throw new Error(
        'PDF extraction failed - no readable content found. The document may be image-based or corrupted.'
      );
    }
    ...
    for (const indicator of binaryIndicators) {
      ...
      if (matches && matches.length > content.length * 0.1) {
        throw new Error(
          'Document contains too much binary data. Please ensure the PDF contains selectable text.'
        );
      }
    }
    ...
    if (content.length > MAX_CHARS) {
      console.log(
        `Content too long (${content.length} chars), truncating to ${MAX_CHARS}`
      );
      content =
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('timeout')) {
          throw new Error(
            'Analysis timed out. Please try with a shorter document.'
          );
        }
        throw error;
      }
  ...
  getCachedAnalysis(
    contentHash: string
  ): Observable<DocumentAnalysisResult | null> {
    return from(this.fetchCachedAnalysis(contentHash));
  }

  private async fetchCachedAnalysis(
    contentHash: string
  ...
  private async fetchAnalysisStats(): Promise<any> {
    try {
      ...
      const averageProcessingTime =
        data.reduce((sum, item) => sum + (item.processing_time_ms || 0), 0) /
        totalAnalyses;
      ...
      const industryCount = industries.reduce(
        (acc: Record<string, number>, industry) => {

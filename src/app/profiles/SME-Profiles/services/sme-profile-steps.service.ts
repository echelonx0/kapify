...
export class SMEProfileStepsService {
  ...
  // ===============================
  // COMPLETION CALCULATIONS (REQUIRED STEPS ONLY)
  // ===============================

  /**
   * Completion percentage based ONLY on required steps
   * Optional steps (business-strategy, review) do not affect this
   */
  ...
  /**
   * Total required steps count (excludes optional steps)
   * Used for progress calculations
   */
  requiredStepsCount = computed(() => {
    return this.steps.filter((step) => step.required).length;
  });

  /**
   * Completed required steps count
   * Used for UI display and progress tracking
   */
  completedRequiredStepsCount = computed(() => {
    return this.steps.filter((step) => step.required && step.completed).length;
  });

  /**
   * Total steps including optional
   * Used for reference/UI display only
   */
  ...
  async loadSavedProfile(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        '⚠️ Failed to load from backend, using local data only:',
        error
      );
  ...
  private async saveToBackend(isManual: boolean = false): Promise<void> {
    ...
    try {
      ...
      console.error(
        `❌ ${isManual ? 'Manual' : 'Auto'} save to backend failed:`,
        error
      );
      if (isManual) {
  ...
  private mergeProfileData(
    localData: Partial<ProfileData>,
    backendData: Partial<ProfileData>
  ): Partial<ProfileData> {
    ...
    try {
      ...
      if (savedData) {
        ...
        if (parsed.userId === user.id) {
          ...
          if (parsed.steps) {
            this.steps.forEach((step) => {
              const savedStep = parsed.steps.find(
                (s: ProfileStep) => s.id === step.id
              );
              if (savedStep) {
  ...
  async submitProfile(): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (incompleteRequired.length > 0) {
        return {
          success: false,
          error: `Please complete the following required sections: ${incompleteRequired
            .map((s) => s.title)
            .join(', ')}`,
        };
      }
  ...
  // Get profile completion summary (required steps only)
  getCompletionSummary(): {
    ...
    const optionalSteps = this.steps.filter((s) => !s.required);

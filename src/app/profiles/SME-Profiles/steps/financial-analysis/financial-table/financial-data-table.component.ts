...
//   // NEW: Support for nested sections (e.g., ASSETS contains Non-Current + Current + Total)
//   children?: FinancialTableSection[];
//   isGroupContainer?: boolean; // Mark as a container for child sections
//   containerLevel?: number; // Indentation level: 0 = parent, 1 = child
//   isHeaderless?: boolean; // Skip rendering section header (show only rows)
// }
...
//   /**
//    * Toggle section by unique key (for nested sections)
//    */
...
//   /**
//    * Generate unique key for section (handles nested sections)
//    */
//   getSectionKey(section: FinancialTableSection, parentKey?: string): string {
//     const key = section.title.replace(/\s+/g, '_').toLowerCase();
...
//   isGroupContainer(section: FinancialTableSection): boolean {
//     return section.isGroupContainer === true;
//   }
...
export class FinancialDataTableComponent implements OnInit {
  ...
  /**
   * Handle cell change with correct index mapping for group containers
   * For group containers: sectionIndex is parent, need to map to child
   * For regular sections: sectionIndex is directly the section
   */
  onCellChange(
    sectionIndex: number,
    rowIndex: number,

...
export class ManagementGovernanceComponent implements OnInit, OnDestroy {
  ...
  // Data - Track initial state for dirty detection
  private initialManagementTeam = signal<LocalManagementMember[]>([]);
  private initialBoardOfDirectors = signal<LocalBoardMember[]>([]);
  private initialManagementCommittee = signal<LocalCommitteeMember[]>([]);
  ...
  isDirty = computed(() => {
    const mgmtChanged =
      ...
      JSON.stringify(this.initialManagementTeam());
    const boardChanged =
      JSON.stringify(this.boardOfDirectors()) !==
      JSON.stringify(this.initialBoardOfDirectors());
    const committeeChanged =
      JSON.stringify(this.managementCommittee()) !==
      JSON.stringify(this.initialManagementCommittee());

    return mgmtChanged || boardChanged || committeeChanged;
  });
  ...
  ngOnInit() {
    ...
    // 🔥 Register dirty signal with StepSaveService
    this.stepSaveService.registerSignalDirty(this.isDirty);

    console.log(
      '✅ ManagementGovernanceComponent initialized with signal-based dirty tracking'
    );
  }
  ...
  ngOnDestroy() {
    ...
    this.stepSaveService.clearDirtySignal();
  }
  ...
  private loadExistingData() {
    ...
    if (existingData) {
      ...
      this.initialManagementTeam.set(
        JSON.parse(JSON.stringify(this.managementTeam()))
      );
      this.initialBoardOfDirectors.set(
        JSON.parse(JSON.stringify(this.boardOfDirectors()))
      );
      this.initialManagementCommittee.set(
        JSON.parse(JSON.stringify(this.managementCommittee()))
      );
    } else {
      this.managementTeam.set([]);
      this.hasBoard.set(true);
  ...
  private async saveData(isManual: boolean = false) {
    ...
    try {
      ...
      // 🔥 Update initial state after successful save (clear dirty flag)
      this.initialManagementTeam.set(
        JSON.parse(JSON.stringify(this.managementTeam()))
      );
      this.initialBoardOfDirectors.set(
  ...
  private buildManagementStructureData(): ManagementStructure {
    ...
    return {
      ...
      managementTeam: managementData.map((mgmt) => ({
        ...
        department: this.getDepartmentFromRole(mgmt.role),
        qualification: mgmt.qualification,
        fieldOfStudy: mgmt.fieldOfStudy,
        yearsOfExperience: mgmt.yearsOfExperience,
        reportsTo: undefined,
      })),

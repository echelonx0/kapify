...
//     this.stepCheckerService.getCompletionSummary()
//   );
//   // Derived data
...
//     this.transformer.transformToFundingProfile(this.profileData())
//   );
...
//     const reviewStep = this.profileService.steps.find(
//       (step) => step.id === 'review'
//     );
//     if (reviewStep) {
...
//     const data = this.profileData();
//     const steps = this.profileService.steps.filter(
//       (step) => step.id !== 'review'
//     );
...
//         status: (businessInfo.companyName ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//       {
...
//         status: (businessInfo.registrationNumber
//           ? 'complete'
...
//         status: (businessInfo.industry ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//       {
...
//           : 'Not provided',
//         status: (personalInfo.firstName ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//     ];
...
//       missingItems.push('Registration number');
...
//         status: (docCount > 0 ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//       {
...
//         status: (docs.companyProfile || docs.cipDocument
//           ? 'complete'
//           : 'incomplete') as 'complete' | 'incomplete',
//       },
//       {
...
//         status: (docs.financialStatements ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//     ];
...
//     const missingItems = [];
//     if (!docs.companyProfile && !docs.cipDocument)
//       missingItems.push('Company registration document');
//     if (!docs.financialStatements) missingItems.push('Financial statements');
//     if (!docs.taxPin && !docs.taxClearance)
//       missingItems.push('Tax compliance documents');
...
//         status: (assessment.businessModel ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//       {
...
//         value:
//           assessment.targetMarkets?.length > 0
//             ? `${assessment.targetMarkets.length} markets`
//             : 'Not specified',
//         status: (assessment.targetMarkets?.length > 0
//           ? 'complete'
//           : 'incomplete') as 'complete' | 'incomplete',
//       },
//       {
...
//         status: (assessment.valueProposition ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//     ];
...
//     const missingItems = [];
//     if (!assessment.businessModel)
//       missingItems.push('Business model description');
//     if (!assessment.targetMarkets || assessment.targetMarkets.length === 0)
//       missingItems.push('Target market definition');
...
//             ? `${swot.strengths.length} identified`
//             : 'None listed',
//         status: (swot.strengths?.length >= 2 ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//       {
...
//             ? `${swot.weaknesses.length} identified`
//             : 'None listed',
//         status: (swot.weaknesses?.length >= 2 ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//       {
...
//             ? `${swot.opportunities.length} identified`
//             : 'None listed',
//         status: (swot.opportunities?.length >= 2
//           ? 'complete'
//           : 'incomplete') as 'complete' | 'incomplete',
//       },
//       {
...
//             ? `${swot.threats.length} identified`
//             : 'None listed',
//         status: (swot.threats?.length >= 2 ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//     ];
...
//     const missingItems = [];
//     if (!swot.strengths || swot.strengths.length < 2)
//       missingItems.push('At least 2 strengths');
//     if (!swot.weaknesses || swot.weaknesses.length < 2)
//       missingItems.push('At least 2 weaknesses');
//     if (!swot.opportunities || swot.opportunities.length < 2)
//       missingItems.push('At least 2 opportunities');
//     if (!swot.threats || swot.threats.length < 2)
//       missingItems.push('At least 2 threats');
...
//         value:
//           management.managementTeam?.length > 0
//             ? `${management.managementTeam.length} members`
//             : 'Not defined',
//         status: (management.managementTeam?.length > 0
//           ? 'complete'
//           : 'incomplete') as 'complete' | 'incomplete',
//       },
//       {
...
//         value:
//           management.boardOfDirectors?.length > 0
//             ? `${management.boardOfDirectors.length} members`
//             : 'Not defined',
//         status: (management.boardOfDirectors?.length > 0
//           ? 'complete'
//           : 'incomplete') as 'complete' | 'incomplete',
//       },
//     ];
...
//         status: (strategy.missionStatement ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//       {
...
//             : 'Not defined',
//         status: (strategy.strategicObjectives?.length > 0
//           ? 'complete'
//           : 'incomplete') as 'complete' | 'incomplete',
//       },
//       {
...
//         status: (strategy.expansionPlans ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//     ];
...
//         value: financialInfo.monthlyRevenue
//           ? `R${financialInfo.monthlyRevenue}`
//           : 'Not provided',
//         status: (financialInfo.monthlyRevenue ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//       {
...
//         status: (financialAnalysis.uploadedFile ? 'complete' : 'warning') as
//           | 'complete'
//           | 'warning',
//       },
//       {
//         label: 'Funding Required',
//         value: fundingInfo.amountRequired
//           ? `R${fundingInfo.amountRequired}`
//           : 'Not specified',
//         status: (fundingInfo.amountRequired ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//       {
...
//         status: (fundingInfo.purposeOfFunding ? 'complete' : 'incomplete') as
//           | 'complete'
//           | 'incomplete',
//       },
//     ];
...
//     if (
//       !financialAnalysis.uploadedFile &&
//       !this.hasFinancialData(financialAnalysis)
//     ) {
//       missingItems.push('Financial projections template');
//     }
//     if (!fundingInfo.amountRequired)
//       missingItems.push('Funding amount required');
...
//     const completeItems = keyData.filter(
...
//         level: 'excellent',
//         message:
//           'Your business profile is comprehensive and ready for funding applications.',
//         recommendations: [
...
//         level: 'good',
//         message:
//           'Your profile is well-developed but could benefit from some improvements.',
//         recommendations: [
...
//         level: 'fair',
//         message:
//           'Your profile needs significant improvements before applying for funding.',
//         recommendations: [
...
//         level: 'poor',
//         message:
//           'Your profile requires substantial work before it will be ready for funding applications.',
//         recommendations: [
...
//     return (
//       financialAnalysis?.incomeStatement?.some((row: any) =>
...
//     link.href = url;
//     link.download = `business_profile_report_${
...
// Status: ${section.completed ? 'Complete' : 'Incomplete'} (${
...
//     return this.sectionSummaries().some(
//       (section) => section.missingItems.length > 0
//     );
//   }

//   getTotalMissingItems(): number {
//     return this.sectionSummaries().reduce(
...
//     const incompleteSections = this.sectionSummaries().filter(
//       (s) => !s.completed
//     );
...
//         'Consider completing the recommended improvements for better funding readiness.'
//       );
//     }
//   }
// }
...
export class ProfileReviewComponent implements OnInit {
  ...
  private stepCheckerService = inject(SMEProfileStepsService);
  private transformer = inject(ProfileDataTransformerService);
  private fundingCoverService = inject(FundingApplicationCoverService);
  ...
  isGeneratingReport = signal(false);
  showAIAnalysis = signal(false);
  showFundingRequiredModal = signal(false);
  ...
  // ===== FUNDING REQUEST CHECK =====
  defaultFundingRequest = computed(() =>
    this.fundingCoverService.defaultProfile()
  );
  hasFundingRequest = computed(() => !!this.defaultFundingRequest());
...
`;
  ...
  /**
   * Check if can start applications
   * NOW INCLUDES: Funding request required
   */
  canStartApplications(): boolean {
    const completion = this.completionSummary();
    const readiness = this.readinessAssessment();
    const hasFunding = this.hasFundingRequest();

    // Must have: profile complete + good readiness + funding request
    return (
      completion.isComplete && readiness.readinessScore >= 70 && hasFunding
    );
  }

  /**
   * Get application readiness message
   * NOW INCLUDES: Funding request check
   */
  getApplicationReadinessMessage(): string {
    const canStart = this.canStartApplications();
    const completion = this.completionSummary();
    ...
    if (!hasFunding) {
      return 'Create a funding request before applying for opportunities.';
    } else if (!completion.isComplete) {
      return 'Complete all required sections before applying for funding.';
    } else if (readiness.readinessScore < 70) {
      return 'Improve your profile quality before applying for funding.';
   ...
   * Blocked if no funding request
   */
  startApplicationProcess() {
    // Check if funding request exists
    if (!this.hasFundingRequest()) {
      this.showFundingRequiredModal.set(true);
      return;
    }

    // Verify all criteria
    if (!this.canStartApplications()) {
      console.warn('Profile not ready for applications');
      return;
    }
  ...
  /**
   * Handle "Create Funding Request" from modal
   */
  createFundingRequest(): void {
    this.showFundingRequiredModal.set(false);

    // Navigate to covers creation
   ...
   * Close modal
   */
  closeFundingRequiredModal(): void {
    this.showFundingRequiredModal.set(false);
  }

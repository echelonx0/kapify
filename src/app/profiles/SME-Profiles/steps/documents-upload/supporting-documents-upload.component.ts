...
export class SupportingDocumentsUploadComponent implements OnInit, OnDestroy {
  ...
  downloadAllDocuments() {
    const uploadedDocs = this.getAllDocuments().filter(
  ...
  previewAllDocuments() {
    const previewableDocs = this.getAllDocuments().filter(
      (doc) => doc.uploaded && !doc.error && this.isPreviewable(doc)
    );
  ...
  getDocumentAnalytics() {
    ...
    return {
      ...
      requiredCompletionRate: Math.round(
        (requiredDocs.filter((doc) => doc.uploaded && !doc.error).length /
          requiredDocs.length) *
          100
      ),
      totalSize: uploadedDocs.reduce(
        (total, doc) => total + (doc.file?.size || 0),
        0
      ),
    };
  }
  ...
  async exportDocumentList(): Promise<void> {
    ...
    a.href = url;
    a.download = `document-upload-summary-${
  ...
  getSectionProgress(section: DocumentSection): number {
    const uploaded = section.documents.filter(
      (doc) => doc.uploaded && !doc.error
    ).length;
    return section.documents.length > 0
      ? Math.round((uploaded / section.documents.length) * 100)
      : 0;
  }
  ...
  hasPreviewableDocuments(): boolean {
    return this.getAllDocuments().some(
      (doc) => doc.uploaded && !doc.error && this.isPreviewable(doc)
    );
  }
  ...
  async processFile(file: File, documentKey: string) {
    ...
    this.supabaseDocumentService
      .uploadDocument(file, documentKey, undefined, category)
      .subscribe({
        next: (result) => {
  ...
  private updateDocument(
    documentKey: string,
    updates: Partial<DocumentUpload>
  ) {
  ...
  private validateFile(file: File): { isValid: boolean; error?: string } {
    ...
    if (file.size > 10 * 1024 * 1024) {
      return {
        isValid: false,
        error: `File size must be less than 10MB. Your file is ${this.formatFileSize(
          file.size
        )}.`,
      };
    }
  ...
  requestHelp() {
    window.open(
      'mailto:support@kapify.com?subject=Document Upload Help',
      '_blank'
    );
  }
  ...
  private loadFromLocalService() {
    const existingData =
      this.fundingApplicationService.data().supportingDocuments;
    if (existingData) {
      this.updateDocumentSectionsFromData(existingData);
    }
  }

  private updateDocumentSectionsFromSupabase(
  ...
  private setupProgressTracking() {
    this.progressSubscription =
      this.supabaseDocumentService.uploadProgress$.subscribe((progressMap) => {
        const activeUploads = Array.from(progressMap.values()).filter(
        ...
        Array.from(progressMap.values())
          ...
          .forEach((progress) => {
            ...
            this.supabaseDocumentService.clearUploadProgress(
              progress.documentKey
            );
          });
  ...
  totalDocuments = () => {
    return this.documentSections().reduce(
  ...
  completedDocuments = () => {
    return this.documentSections().reduce(
      (total, section) =>
        total +
        section.documents.filter((doc) => doc.uploaded && !doc.error).length,
      0
    );
  };
  ...
  totalRequiredDocuments = () => {
    return this.documentSections().reduce(
      (total, section) =>
        total + section.documents.filter((doc) => doc.required).length,
      0
    );
  };
    ...
    return this.documentSections().reduce(
      (total, section) =>
        ...
        section.documents.filter(
          (doc) => doc.required && doc.uploaded && !doc.error
        ).length,
      0
    );
  };
  ...
  private openImagePreviewModal(file: File) {
    ...
    container.className =
      'relative max-w-4xl max-h-full bg-white rounded-lg overflow-hidden';
    ...
    closeBtn.innerHTML = '×';
    closeBtn.className =
      'absolute top-4 right-4 text-white bg-black bg-opacity-50 rounded-full w-8 h-8 flex items-center justify-center text-xl hover:bg-opacity-75';
  ...
  private truncateFileName(fileName: string, maxLength: number): string {
    ...
    if (nameWithoutExt.length > maxNameLength) {
      return `${nameWithoutExt.substring(0, maxNameLength)}...${
        extension ? '.' + extension : ''
      }`;
    }

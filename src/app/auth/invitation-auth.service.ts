...
export class InvitationAuthService {
  ...
  private async fetchInvitationDetails(
    ...
    // Extract organization name (handle possible array from FK)
    const orgName = Array.isArray(invitation.organizations)
      ? invitation.organizations[0]?.name
      : '';

    // Extract inviter name (handle possible array from FK)
    const inviterRecord = Array.isArray(invitation.users)
      ? invitation.users[0]
      : invitation.users;

...
//     const type = this.authService.user()?.userType;
//     return (type === 'funder' ? 'funder' : 'sme') as 'sme' | 'funder';
//   });
...
//       this.allFAQs()
//         .filter(
//           (f) => f.user_type === this.userType() || f.user_type === 'both'
//         )
//         .map((f) => f.category)
//     );
...
//       .filter((f) => f.user_type === this.userType() || f.user_type === 'both')
...
//     this.faqService.getFAQsByUserType(this.userType()).subscribe({
//       next: (faqs) => {
...
export class FAQsComponent implements OnInit {
  ...
  readonly categories = computed(() => {
    const cats = new Set(this.allFAQs().map((f) => f.category));
    return Array.from(cats).sort();
  });

...
/**
 * AdminService
 * FIXED: Explicit foreign key relationships to resolve ambiguity
...
export class AdminService implements OnDestroy {
   ...
   * FIXED: Explicit foreign key relationship hints
   */
  getAllUsers(
  ...
  /**
   * Fetch users with explicit foreign key hints
   */
      ...
      .select(
        ...
        *,
        ...
        organization_users!fk_organization_users_user (
          organization_id,
          organizations!fk_organization_users_organization (name)
        )
      `
      )
   ...
   * FIXED: Explicit foreign key hints
   */
  getAllOrganizations(
    options: PaginationOptions = { page: 1, pageSize: 50 }
  ...
  /**
   * Fetch organizations with explicit foreign key hints
   */
  private async fetchAllOrganizations(
    ...
    const { data: orgs, error } = await this.supabase
      ...
      .select(
        `
        *,
        organization_users!fk_organization_users_organization (
          role,
   ...
   * FIXED: No join with organizations (missing FK)
   */
  getAllOpportunities(
    options: PaginationOptions = { page: 1, pageSize: 50 }

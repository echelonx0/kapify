...
tabs = computed(() => [
  ...
  toggleUserStatus(user: AdminUser) {
    ...
    this.adminService.updateUserStatus(user.id, newStatus).subscribe({
      next: (updatedUser) => {
        ...
        console.log(`User ${user.firstName} ${user.lastName} ${newStatus === 'active' ? 'activated' : 'suspended'} successfully`);
      },
      error: (error) => {
  ...
  resetUserPassword(user: AdminUser) {
    if (confirm(`Reset password for ${user.firstName} ${user.lastName}? They will need to use the temporary password to log in.`)) {
  ...
  verifyOrganization(org: AdminOrganization) {
    if (confirm(`Verify ${org.name}? This will mark the organization as verified and enable full platform access.`)) {
    ...
    if (confirm(`Are you sure you want to ${action} ${org.name}?`)) {
      this.adminService.updateOrganizationStatus(org.id, newStatus).subscribe({
        ...
        error: (error) => {
          ...
          this.error.set('Failed to update organization status. Please try again.');
        }
      });
    }
  }
  ...
  toggleOpportunityStatus(opportunity: AdminOpportunity) {
    ...
    if (confirm(`${action.charAt(0).toUpperCase() + action.slice(1)} "${opportunity.title}"?`)) {
      this.adminService.updateOpportunityStatus(opportunity.id, newStatus).subscribe({
        next: (updatedOpp) => {
        ...
        error: (error) => {
          ...
          this.error.set('Failed to update opportunity status. Please try again.');
        }
      });
    }
  }
  ...
  deleteOpportunity(opportunity: AdminOpportunity) {
    if (confirm(`Delete "${opportunity.title}"? This action cannot be undone and will affect ${opportunity.applicationsCount || 0} applications.`)) {
  ...
  getEnhancedStatusBadge(status: string): string {
    const badges = {
      'active': 'inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-primary-100 to-primary-200 text-primary-800 shadow-sm',
      'inactive': 'inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-neutral-100 to-neutral-200 text-neutral-700 shadow-sm',
      ...
      'pending_verification': 'inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-warning/20 to-warning/30 text-warning shadow-sm',
      'verified': 'inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 shadow-sm',
      'draft': 'inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-neutral-100 to-neutral-200 text-neutral-600 shadow-sm',
      'paused': 'inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-warning/20 to-warning/30 text-warning shadow-sm',
      'closed': 'inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-neutral-200 to-neutral-300 text-neutral-700 shadow-sm'
    };
  ...
  sortOpportunitiesByDate(opportunities: AdminOpportunity[]): AdminOpportunity[] {
  ...
  getAriaLabel(item: any, type: 'user' | 'organization' | 'opportunity'): string {
    switch (type) {
      case 'user':
  ...
  private convertToCSV(data: any[], columns: { key: string; label: string }[]): string {

...
export class BusinessRulesAnalysisService {
  ...
  analyzeApplication(
    ...
    const result = this.performProfileAnalysis(
      profile,
      opportunity,
      applicationData
    );
  ...
  private performProfileAnalysis(
    ...
    const industryAlignment = this.analyzeIndustryAlignment(
      profile,
      opportunity
    );
    const stageCompatibility = this.analyzeStageCompatibility(
      profile,
      opportunity
    );
    const financialReadiness = this.analyzeFinancialReadiness(
      profile,
      opportunity
    );
  ...
  private isAmountEligible(
    ...
    return (
      requestedAmount >= opportunity.minInvestment &&
  ...
  private createAmountRejectionResult(
    ...
    if (isNaN(requestedAmount) || requestedAmount <= 0) {
      ...
      rejectionReason = `Requested amount (${this.formatCurrency(
        requestedAmount
      )}) is below minimum investment of ${this.formatCurrency(
        opportunity.minInvestment
      )}`;
    } else {
      rejectionReason = `Requested amount (${this.formatCurrency(
        requestedAmount
      )}) exceeds maximum investment of ${this.formatCurrency(
    ...
    return {
      ...
      industryAlignment: {
        score: 0,
        match: 'none',
        details: 'Amount eligibility failed',
      },
      stageCompatibility: {
        score: 0,
        match: 'weak',
      ...
      strengths: [],
      improvementAreas: [
        `Adjust funding amount to between ${this.formatCurrency(
          opportunity.minInvestment
        )} and ${this.formatCurrency(opportunity.maxInvestment)}`,
      ],
      recommendations: [
  ...
  private analyzeIndustryAlignment(
    ...
    if (!opportunity) {
      ...
      if (!industry) {
        return {
          score: 10,
          match: 'none',
          details: 'Industry not specified in profile',
        };
      }
      return {
        score: 30,
        match: 'strong',
        details: `Industry clearly defined: ${industry}`,
      };
    }
    ...
    if (targetIndustries.length === 0) {
      return {
        score: 25,
        match: 'moderate',
        details: 'Opportunity accepts all industries',
      };
    }

    // Direct match
    const directMatch = targetIndustries.some(
      (target) =>
    ...
    if (directMatch) {
      return {
        score: 30,
        match: 'strong',
        details: `Strong industry alignment: ${profileIndustry}`,
      };
    }

    // Related industry logic
    const relatedMatch = this.checkRelatedIndustries(
      profileIndustry,
      targetIndustries
    );
    if (relatedMatch) {
      return {
        score: 20,
        match: 'moderate',
        details: `Related industry match: ${profileIndustry}`,
      };
    }
    ...
    return {
      score: 5,
      match: 'weak',
      details: `Industry mismatch: ${profileIndustry} not in target industries`,
    };
  }
  ...
  private analyzeStageCompatibility(
    ...
    const businessStage = this.determineBusinessStage(
      profile,
      yearsInOperation
    );
    ...
    if (targetStages.length === 0) {
      return {
        score: 20,
        match: 'moderate',
        details: 'Opportunity accepts all business stages',
      };
    }

    const stageMatch = targetStages.some(
      (stage) => stage.toLowerCase() === businessStage.toLowerCase()
    );
    ...
    if (stageMatch) {
      return {
        score: 25,
        match: 'strong',
        details: `Perfect stage match: ${businessStage}`,
      };
    }
    ...
    if (adjacentMatch) {
      return {
        score: 15,
        match: 'moderate',
        details: `Adjacent stage compatibility: ${businessStage}`,
      };
    }
    ...
    return {
      score: 5,
      match: 'weak',
      details: `Stage mismatch: ${businessStage} not suitable for this opportunity`,
    };
  }
  ...
  private analyzeFinancialReadiness(
    ...
    const annualRevenue = profile.financialProfile?.monthlyRevenue
      ? profile.financialProfile.monthlyRevenue * 12
      : 0;
    ...
    if (opportunity) {
      ...
      if (minRevenue && annualRevenue < minRevenue) {
        ...
        details += ` • Below minimum revenue requirement (${this.formatCurrency(
          minRevenue
        )})`;
      }
  ...
  private generateInsights(
    ...
    BusinessRulesResult,
    'strengths' | 'improvementAreas' | 'recommendations' | 'riskFlags'
  > {
    ...
    if (profileCompleteness.missingCriticalSections.length > 0) {
      improvementAreas.push(
        `Complete critical sections: ${profileCompleteness.missingCriticalSections.join(
          ', '
        )}`
      );
      recommendations.push(
        'Focus on completing critical profile sections first for better funding readiness'
      );
    }

    if (financialReadiness.score < 10) {
      improvementAreas.push(
        'Strengthen financial documentation and performance metrics'
      );
      recommendations.push(
        'Prepare comprehensive financial statements and projections'
      );
    ...
    if (industryAlignment.score < 15 && opportunity) {
      improvementAreas.push(
        'Consider opportunities that better align with your industry'
      );
      recommendations.push(
        'Look for funders specializing in your industry sector'
      );
    }
    ...
    if (opportunity) {
      recommendations.push(
        'Review opportunity-specific requirements thoroughly'
      );
      if (stageCompatibility.score < 15) {
        recommendations.push(
          'Consider if your business stage aligns with funder expectations'
        );
      }
    } else {
      recommendations.push(
        'Complete any missing profile sections to improve funding readiness'
      );
      recommendations.push(
        'Consider obtaining professional financial projections if not available'
      );
    }

    // Add compliance/operational risks
    if (
      !profile.companyInfo?.taxComplianceStatus ||
      profile.companyInfo.taxComplianceStatus !== 'compliant'
    ) {
      riskFlags.push({
        category: 'compliance',
        severity: 'high',
  ...
  private calculateYearsInOperation(
    profile: FundingApplicationProfile
  ): number {
  ...
  private checkRelatedIndustries(
      ...
      if (related.some((r) => profileIndustry.includes(r))) {
        return targetIndustries.some(
          (target) =>
            ...
            related.includes(target.toLowerCase())
        );
      }
    }
  ...
  private checkFinancialDocuments(profile: FundingApplicationProfile): {
    ...
    return {
      current: !!(
        docs?.currentYearFinancials || docs?.auditedFinancials?.length
      ),
      historical: !!(
        docs?.priorYearFinancialYear1 || docs?.priorYearFinancialYear2
      ),
    };
  }

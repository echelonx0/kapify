...
//       // Filter by funder_id (organization ID) if provided
//       if (funderId) {
...
export class KapifyReportsRepositoryService {
   ...
   * Fetch applications with all related data
   * Optimized with batch operations and proper null handling
   * @param funderId - REQUIRED: Organization/funder ID to filter applications
   */
  async getApplicationsWithData(
    funderId: string,
    filters?: KapifyReportsFilter,
   ...
   * @param funderId - REQUIRED: Organization/funder ID for authorization check
   */
  async getSingleApplicationWithData(
    applicationId: string,
    funderId: string
    ...
    try {
      if (!funderId) {
        throw new Error('funderId is required for authorization');
      }
      ...
      const { data: appDataRaw, error: appError } = await this.supabase
        ...
        .select('*')
        .eq('id', applicationId)
        .eq('funder_id', funderId) // Ensure application belongs to this funder
        .single();

      if (appError || !appDataRaw) {
        throw new Error(
          'Application not found or does not belong to this funder'
        );
      }
  ...
  async getApplicationsByOpportunity(
    ...
    try {
      ...
      console.log(
        '🔍 [REPO] Fetching applications by opportunity:',
        opportunityId
      );
      console.log('   Funder ID:', funderId);

      // Fetch applications for this opportunity and funder (raw from DB)
      const { data: applicationsRaw, error } = await this.supabase
        .from('applications')
        .select('*')
        .eq('opportunity_id', opportunityId)
   ...
   * STRICT: funderId is REQUIRED
   * @param funderId - REQUIRED: Organization/funder ID to filter applications
   * @param filters - Optional filters
   * @param excludeStatuses - Status values to exclude (default: draft, withdrawn)
   */
  private async fetchApplicationsRaw(
    funderId: string,
    filters?: KapifyReportsFilter,
    ...
    try {
      // STRICT: funderId validation
      if (!funderId || funderId.trim() === '') {
        throw new Error(
          'funderId is required and cannot be empty. All applications must be scoped to a funder.'
        );
      }

...
// src/app/funders/components/financial-analysis-viewer/financial-analysis-viewer.component.ts
import { Component, Input, computed } from '@angular/core';
import { CommonModule } from '@angular/common';
import {
  LucideAngularModule,
...
export class FinancialAnalysisViewerComponent {
  ...
  hasData = computed(() => {
    const data = this.financialAnalysis;
    console.log('🔍 [VIEWER] Checking hasData:', data);
    console.log(
      '🔍 [VIEWER] Income statement length:',
      data?.incomeStatement?.length
    );
    console.log(
      '🔍 [VIEWER] Balance sheet length:',
      data?.balanceSheet?.length
    );
    console.log('🔍 [VIEWER] Cash flow length:', data?.cashFlow?.length);
  ...
  uploadedFile = computed(() => {
    const file = this.financialAnalysis?.uploadedFile;
    console.log('🔍 [VIEWER] Uploaded file:', file);
    return file;
  });

  columnHeaders = computed(() => {
    const headers = this.financialAnalysis?.columnHeaders || [];
    console.log('🔍 [VIEWER] Column headers:', headers);
    return headers;
  });
  ...
  summaryMetrics = computed(() => {
    const data = this.financialAnalysis;
    if (!data) {
      console.log('⚠️ [VIEWER] No data for summary metrics');
      return null;
    }
    ...
    console.log('📊 [VIEWER] Summary metrics:', metrics);
    return metrics;
  });
  ...
  keyRatios = computed(() => {
    ...
    if (!ratios || ratios.length === 0) {
      console.log('⚠️ [VIEWER] No financial ratios');
      return [];
    }

    const lastPeriodIndex = this.columnHeaders().length - 1;
    if (lastPeriodIndex < 0) {
      console.log('⚠️ [VIEWER] No column headers for ratios');
      return [];
    }
    ...
    console.log('📊 [VIEWER] Key ratios:', result);
    return result;
  });
  ...
  revenueTrend = computed(() => {
    ...
    if (!income || income.length === 0) {
      console.log('⚠️ [VIEWER] No income statement for revenue trend');
      return null;
    }
    ...
    if (!revenueRow) {
      console.log('⚠️ [VIEWER] No revenue row found');
      return null;
    }
    ...
    const lastIndex = headers.length - 1;
    if (lastIndex < 2) {
      console.log('⚠️ [VIEWER] Not enough periods for trend');
      return null;
    }
    ...
    console.log('📈 [VIEWER] Revenue trend:', trend);
    return trend;
  });
  ...
  revenueGrowth = computed(() => {
    ...
    if (!trend || trend.previous === 0) {
      console.log('⚠️ [VIEWER] Cannot calculate revenue growth');
      return null;
    }
    ...
    console.log('📈 [VIEWER] Revenue growth:', result);
    return result;
  });

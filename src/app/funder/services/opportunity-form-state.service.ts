...
//   // Form data updates
//   updateField(field: keyof OpportunityFormData, value: string | boolean) {
...
//   updateMultiSelectField(field: keyof OpportunityFormData, value: string, checked: boolean) {
//     this.formData.update(data => {
...
//   onNumberInput(field: keyof OpportunityFormData, value: string): void {
...
//       typicalInvestment: draftData.totalAvailable?.toString() || '',
...
export class OpportunityFormStateService { 
  ...
  loadFromDraft(draftData: any) {
    this.formData.set({
      ...
      geographicRestrictions: draftData.eligibilityCriteria?.geographicRestrictions || [],
      requiresCollateral: draftData.eligibilityCriteria?.requiresCollateral || false,
      // UPDATED: Maps from backend field (adjust if backend uses different name)

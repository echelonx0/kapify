...
//   currentStep: number;
//   totalSteps: number; // Always 2 (org-info + legal-compliance)
...
//   // ✅ ONLY 2 STEPS - Verification is separate action, not counted
...
//       this.toastService.error(`Validation failed: ${validation.errors[0]}`);
...
//       this.toastService.error('No organization found - please contact support');
...
//    * ✅ FIXED: Uses finalize() to reset isSaving in all paths
//    */
...
//       catchError((error) => {
//         this.error.set(error.message || 'Save failed');
//         this.toastService.error(error.message || 'Save failed');
//         return throwError(() => error);
//       }),
//       // ✅ FIXED: finalize() ensures isSaving is reset in ALL paths (success & error)
//       finalize(() => {
//         this.isSaving.set(false);
//       }),
//     );
...
//    * ✅ Called AFTER onboarding is complete (separate from step progression)
//    */
//   requestVerification(): Observable<{ success: boolean; message: string }> {
//     if (!this.isOnboardingComplete()) {
//       this.toastService.error('Complete all onboarding steps first');
//       return throwError(() => new Error('Complete all onboarding steps first'));
//     }
...
//     if (!orgId) {
//       this.toastService.error('Organization not found');
...
//         if (error) {
//           this.toastService.error(
//             `Verification request failed: ${error.message}`,
//           );
...
//     // ✅ Current step only advances through 2 steps
//     let currentStepIndex = 0;
//     if (basicComplete && !legalComplete) currentStepIndex = 1;
//     else if (basicComplete && legalComplete) currentStepIndex = 2; // Beyond last step = complete

//     // ✅ Completion = both steps done (100% when legalComplete)
...
//       totalSteps: 2, // ✅ Always 2
//       completionPercentage,
//       organization: data as Organization,
//       isComplete: basicComplete && legalComplete, // ✅ Complete when both done
...
//   /**
//    * ✅ Renamed: isReadyForVerification → isOnboardingComplete
//    * Returns true when both steps are done
//    */
...
//     const totalSteps = steps.length; // Always 2
...
export class FunderOnboardingService {
  ...
  // ✅ organizationType removed - immutable at registration, never stored locally
  organizationData = signal<Partial<Organization>>({
    country: 'South Africa',
    status: 'active',
    isVerified: false,
  ...
  private steps: OnboardingStep[] = [
    {
      ...
      title: 'Basic Information',
      shortTitle: 'Basic Info',
      description: 'Organization name, description, and contact details',
      completed: false,
      required: true,
      estimatedTime: '5 min',
    },
  ...
  /**
   * Update organization data and persist locally
   * ✅ organizationType is excluded from all updates
   */
  updateOrganizationData(updates: Partial<Organization>) {
    // Filter out organizationType - it's immutable
    const { organizationType, ...safeUpdates } = updates;
  ...
  /**
   * Save to database via funder_organization_id FK
   * ✅ FIXED: organizationType is excluded from all save operations
   */
  saveToDatabase(): Observable<{ success: boolean; organizationId?: string }> {
    const user = this.authService.user();
    if (!user) {
   ...
   * Update existing organization in database
   * ✅ FIXED: Uses finalize() to reset isSaving in all paths
   * ✅ organizationType is excluded from updates
   */
  private updateExistingOrganization(
    orgId: string,
    data: Partial<Organization>,
    ...
    return from(
      ...
      tap(({ data: orgResult, error: orgError }) => {
        ...
        // Update local state with result (excludes organizationType)
        const updatedData = this.mapDatabaseToModel(orgResult);
        this.organizationData.update((current) => ({
          ...current,
          ...updatedData,
  ...
  /**
   * ✅ organizationType check removed - it's immutable at registration
   */
  isBasicInfoValid(): boolean {
    const data = this.organizationData();
    return !!(
      ...
      if (saved) {
        ...
        if (parsedData.userId === user.id) {
          this.organizationData.set({
            ...parsedData.organizationData,
            country: parsedData.organizationData.country || 'South Africa',
            // ✅ organizationType explicitly excluded from restoration
          });
          this.currentStep.set(parsedData.currentStep || 'organization-info');
          if (parsedData.lastSaved) {
            this.lastSavedLocally.set(new Date(parsedData.lastSaved));
  ...
  private mapDatabaseToModel(dbOrg: any): Partial<Organization> {
    return {
      ...
      name: dbOrg.name,
      description: dbOrg.description,
      // ✅ organizationType explicitly excluded from mapping
      status: dbOrg.status,
      website: dbOrg.website,
      logoUrl: dbOrg.logo_url,
      legalName: dbOrg.legal_name,
  ...
  /**
   * ✅ organizationType is excluded from validation
   */
  private validateDataForDatabase(data: Partial<Organization>): {
    isValid: boolean;
    errors: string[];

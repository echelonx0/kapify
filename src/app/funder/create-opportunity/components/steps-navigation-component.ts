...
@Component({
  ...
  template: `
    <div
      class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 sticky top-6"
    >
      <!-- Progress Header -->
      <div class="mb-6">
...
export class OpportunityStepsNavigationComponent {
  ...
  allStepsCompleted = computed(() => {
    return this.steps.every((step) =>
      this.stepNavigation.isStepCompleted(step.id)
    );
  });
  ...
  getStepClasses(stepId: string): string {
    ...
    if (isCompleted) {
      return 'bg-teal-50 hover:bg-teal-100 border border-transparent';
    }
  ...
  getTitleClasses(stepId: string): string {
    ...
    if (isCompleted) {
      return 'text-sm font-medium text-teal-700';
    }

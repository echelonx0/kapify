...
export class FundingOpportunityImportService {
  ...
  private readonly FIELD_MAPPINGS: ImportFieldMapping[] = [
    ...
    {
      sourceField: '',
      targetField: 'repaymentTerms',
      required: false,
      dataType: 'string',
    },
    {
      sourceField: '',
      targetField: 'securityRequired',
      required: false,
      dataType: 'string',
    },
    {
      sourceField: '',
      targetField: 'useOfFunds',
      required: false,
      dataType: 'string',
    },
    {
      sourceField: '',
      targetField: 'investmentStructure',
      required: false,
      dataType: 'string',
    },
    {
      sourceField: '',
      targetField: 'exitStrategy',
      required: false,
      dataType: 'string',
    },
  ];
        ...
        if (!mapping.sourceField) {
          if (mapping.required) {
            errors.push({
              ...
              value: null,
              message: `${this.getFieldDisplayName(
                mapping.targetField
              )} is required but not mapped`,
              severity: 'error',
            });
          }
          continue;
  ...
  private async bulkImportToDatabase(
    userId: string,
    validatedData: any[]
  ): Promise<ImportResult> {
    console.log(
      `🚀 Starting bulk import of ${validatedData.length} opportunities`
    );
    ...
    for (let i = 0; i < validatedData.length; i++) {
      ...
      try {
        ...
        const dbOpportunity = this.transformForDatabase(
          userId,
          organizationId,
          opportunity
        );
        ...
        console.log(
          `Importing opportunity ${i + 1}/${validatedData.length}: ${
            opportunity.title
          }`
        );
  ...
  private async getOrganizationId(userId: string): Promise<string> {
    ...
    if (error || !org) {
      throw new Error(
        'Organization not found. Please complete your organization setup first.'
      );
    }
  ...
  private transformForDatabase(
    ...
    return {
      ...
      max_investment: opportunity.maxInvestment
        ? Math.max(opportunity.maxInvestment, opportunity.minInvestment || 0)
        : 0,
      ...
      application_deadline:
        opportunity.applicationDeadline?.toISOString() || null,
      ...
      max_applications: opportunity.maxApplications
        ? Math.max(1, opportunity.maxApplications)
        : null,
    ...
    return [
      {
        ...
        description:
          'Funding for established technology companies looking to expand operations, enter new markets, or develop new products',
      ...
      },
      {
        ...
        description:
          'Short-term financing to help small and medium enterprises manage cash flow and operational expenses',

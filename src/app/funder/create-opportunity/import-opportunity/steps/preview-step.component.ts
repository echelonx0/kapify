...
@Component({
  ...
  template: `
    <div class="p-8">
      ...
      <!-- Status Alert -->
      @if (validationSummary()) { @if (canImport()) {
        ...
        <div class="flex items-start space-x-3">
          ...
          <div>
            ...
            <p class="text-sm text-green-700 mt-1">
              {{ validationSummary()!.validRows }} opportunities are ready to be
              imported. @if (validationSummary()!.warningRows > 0) {
              {{ validationSummary()!.warningRows }} rows have warnings but can
              still be imported. }
            </p>
          </div>
        </div>
        ...
        <div class="flex items-start space-x-3">
          ...
          <div>
            ...
            <p class="text-sm text-red-700 mt-1">
              {{ validationSummary()!.errorRows }} rows have errors that must be
              fixed before importing. Please review and correct the data in your
              source file.
            </p>
          </div>
        </div>
        ...
        <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
          <div class="flex items-center justify-between">
            ...
            <div class="text-sm text-gray-600">
              Showing {{ Math.min(previewData().length, maxPreviewRows) }} of
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              @for (row of previewData().slice(0, maxPreviewRows); track $index;
              let i = $index) {
              <tr
                [class.bg-red-50]="hasRowErrors(i)"
                [class.bg-yellow-50]="hasRowWarnings(i) && !hasRowErrors(i)"
              >
                ...
                <td class="px-4 py-2 text-sm text-gray-900">
                  {{
                    Array.isArray(row.fundingType)
                      ? row.fundingType.join(', ')
                      : row.fundingType || '-'
                  }}
                </td>
                <td class="px-4 py-2 text-sm text-gray-900">
      ...
      <div class="border border-gray-200 rounded-lg overflow-hidden">
        <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
          ...
          <p class="text-sm text-gray-600 mt-1">
            Review and fix these issues in your source file
          </p>
        </div>

        <div class="max-h-64 overflow-y-auto">
          <div class="divide-y divide-gray-200">
            @for (error of validationSummary()!.errors.slice(0, 50); track
            $index) {
            <div
              class="p-4"
              [class.bg-red-50]="error.severity === 'error'"
              [class.bg-yellow-50]="error.severity === 'warning'"
            >
              <div class="flex items-start space-x-3">
                <lucide-angular
                  [img]="AlertCircleIcon"
            ...
            <div class="p-4 bg-gray-50 text-center">
              <p class="text-sm text-gray-600">
                Showing first 50 of
                {{ validationSummary()!.errors.length }} issues. Fix these
                issues to see more.
              </p>
            </div>
            }
...
export class PreviewStepComponent implements OnInit {
  ...
  private processTransformedData() {
    if (
      !this.transformedData?.mappings ||
      !this.transformedData?.uploadedData
    ) {
      return;
    }
  ...
  private validateFieldValues(
    ...
    Object.keys(row).forEach((field) => {
      ...
      const errorMessage = this.validationService.validateFieldValue(
        field,
        value
      );
      if (errorMessage) {
  ...
  private validateRowConstraints(
    ...
    if (
      row.minInvestment &&
      row.maxInvestment &&
      row.minInvestment > row.maxInvestment
    ) {
      errors.push({
        row: rowIndex,
        field: 'maxInvestment',
    ...
    if (
      row.offerAmount &&
      row.totalAvailable &&
      row.offerAmount > row.totalAvailable
    ) {
      errors.push({
        row: rowIndex,
        field: 'offerAmount',
  ...
  private calculateValidationSummary(
    ...
    const warningRows = new Set(
      errors.filter((e) => e.severity === 'warning').map((e) => e.row)
    );
  ...
  hasRowErrors(rowIndex: number): boolean {
    const summary = this.validationSummary();
    return summary
      ? summary.errors.some((e) => e.row === rowIndex && e.severity === 'error')
      : false;
  }
  ...
  hasRowWarnings(rowIndex: number): boolean {
    ...
    return summary
      ? summary.errors.some(
          (e) => e.row === rowIndex && e.severity === 'warning'
        )
      : false;
  }

// // src/app/funding/funding-detail/funding-detail.component.ts
...
export class OpportunityDetailsComponent implements OnInit {
  ...
  /**
   * Load organization's public profile by ID
   * Stores profile data and checks if it has been published
   */
  private loadPublicProfile(organizationId: string) {
    this.isLoadingProfile.set(true);
    ...
    this.publicProfileService
      ...
      .subscribe({
        next: (profile) => {
          ...
          this.hasPublicProfile.set(!!profile && profile.isPublished);
          this.isLoadingProfile.set(false);
        },
        error: (error) => {
          console.log(
            'Public profile not available for this organization',
            error
          );
  ...
  /**
   * View investor's public profile in new window
   * Uses the profile slug to create shareable public URL
   */
  viewInvestorProfile() {
    const profile = this.profile();
    if (profile?.slug) {
      window.open(`/funder/${profile.slug}`, '_blank');
    }
  }

      ...
      <div class="flex-1 overflow-y-auto space-y-4 p-4 max-h-96">
        @if (comments().length === 0) {
          <div class="text-center py-8">
            <lucide-icon [img]="MessageSquareIcon" [size]="48" class="text-neutral-300 mx-auto mb-3" />
            <h3 class="text-sm font-medium text-neutral-900 mb-1">No comments yet</h3>
            <p class="text-sm text-neutral-500">
              @if (currentUserRole() === 'funder') {
                Add internal review notes for this section
              } @else {
                Comments from your funder will appear here
              }
            </p>
          </div>
        } @else {
              ...
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2">
                  <div class="w-6 h-6 bg-primary-100 rounded-full flex items-center justify-center">
                    <lucide-icon [img]="UserIcon" [size]="14" class="text-primary-600" />
                  </div>
                  <div class="text-xs">
                    <span class="font-medium text-neutral-900">{{ comment.userName }}</span>
                    <span class="text-neutral-500 ml-1">({{ getRoleLabel(comment.userRole) }})</span>
                  </div>
...
export class SectionCommentsComponent implements OnInit {
  ...
  addComment() {
    if (this.commentForm.valid && this.currentUserRole() === 'funder') {
      ...
      const newComment: Omit<SectionComment, 'id' | 'createdAt' | 'userId' | 'userName' | 'userRole'> = {
  ...
  private loadComments() {
    this.commentsService.getSectionComments(this.applicationId(), this.sectionId()).subscribe({
      next: (comments) => {

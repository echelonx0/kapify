...
export class DataRoomComponent implements OnInit {
  ...
  // ✅ FIXED: Return direct values, not functions
  ...
  // ✅ FIXED: Return array directly, not function
  sections = computed<DataRoomSection[]>(() => {
    return this.dataRoomView()?.sections || [];
  });

  // ✅ FIXED: Return dataRoom directly, not function
  dataRoom = computed<DataRoom | null>(() => {
    return this.dataRoomView()?.dataRoom || null;
  });

  // ✅ FIXED: Return object directly, not function
  companyInfo = computed(() => {
    const profile = this.profileData();
    if (!profile) return null;
  ...
  // ✅ FIXED: Return value directly, not function
  fundingInfo = computed(() => {
    const profile = this.profileData();
  ...
  // ✅ FIXED: Return string directly, not function

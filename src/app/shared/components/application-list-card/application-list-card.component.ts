...
export class ApplicationListCardComponent {
  ...
  getFundingTypeTags(): string[] {
    if (
      !this.application.fundingType ||
      this.application.fundingType.length === 0
    ) {
      return [];
    }
    return this.application.fundingType.map(
      (type) =>
  ...
  getFundingTypeColor(type: string): string {
    ...
    return (
      colorMap[type] || 'bg-slate-100 text-slate-700 border border-slate-200'
    );
  }

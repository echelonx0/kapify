...
import { ParsedFinancialData } from 'src/app/SMEs/profile/services/excel-parser.service';
      ...
      <div class="flex min-h-screen items-center justify-center p-4">
        ...
        >
          ...
          <div class="flex items-center justify-between p-6 border-b border-gray-200">
            <div class="flex items-center space-x-3">
              <div class="w-12 h-12 bg-primary-100 rounded-lg flex items-center justify-center">
                <lucide-icon [name]="ActivityIcon" [size]="24" class="text-primary-600" />
              </div>
              <div>
                <h2 class="text-2xl font-bold text-gray-900">Financial Intelligence Analysis</h2>
                <p class="text-sm text-gray-600">Automated insights from your financial data</p>
              </div>
            </div>
            <button 
              (click)="close()"
          ...
          <div class="flex-1 overflow-y-auto p-6">
            @if (analysisReport()) {
              <!-- Health Score Banner -->
              <div class="mb-6 p-6 rounded-xl" [ngClass]="getHealthScoreColor(analysisReport()!.healthScore)">
                <div class="flex items-center justify-between mb-4">
                  <div>
                    <h3 class="text-lg font-semibold text-gray-900">Financial Health Score</h3>
                    <p class="text-sm text-gray-600">{{ analysisReport()!.periodCovered }}</p>
                  </div>
                  <div class="text-right">
                    ...
                    <div class="text-xl font-semibold mt-1" [ngClass]="getGradeColor(analysisReport()!.healthScore.grade)">
                ...
                <div class="grid grid-cols-4 gap-4 mt-4">
                  <div class="text-center">
                    <div class="text-2xl font-bold text-gray-900">{{ analysisReport()!.healthScore.breakdown.profitability }}</div>
                    <div class="text-xs text-gray-600 mt-1">Profitability</div>
                  </div>
                  <div class="text-center">
                    <div class="text-2xl font-bold text-gray-900">{{ analysisReport()!.healthScore.breakdown.liquidity }}</div>
                    <div class="text-xs text-gray-600 mt-1">Liquidity</div>
                  </div>
                  <div class="text-center">
                    <div class="text-2xl font-bold text-gray-900">{{ analysisReport()!.healthScore.breakdown.efficiency }}</div>
                    <div class="text-xs text-gray-600 mt-1">Efficiency</div>
                  </div>
                  <div class="text-center">
                    <div class="text-2xl font-bold text-gray-900">{{ analysisReport()!.healthScore.breakdown.growth }}</div>
              ...
              <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <p class="text-sm text-blue-800">{{ analysisReport()!.executiveSummary }}</p>
              </div>
              ...
              @if (analysisReport()!.validationIssues.length > 0) {
                <div class="mb-6">
                  <div class="flex items-center justify-between mb-3">
                    ...
                    <span class="px-3 py-1 rounded-full text-sm font-semibold" [ngClass]="getQualityBadgeColor(analysisReport()!.dataQualityScore)">
                      {{ analysisReport()!.dataQualityScore }}% Quality Score
                    </span>
                  </div>
                  <div class="space-y-2">
                    @for (issue of analysisReport()!.validationIssues; track issue.field) {
                      <div class="flex items-start space-x-3 p-3 rounded-lg" [ngClass]="getIssueBgColor(issue.severity)">
                        <lucide-icon 
                        ...
                        />
                        <div class="flex-1">
                          <p class="text-sm font-medium" [class]="getIssueTextColor(issue.severity)">
                            {{ issue.field }}
                          </p>
                          <p class="text-sm mt-1" [class]="getIssueTextColor(issue.severity)">
                            {{ issue.message }}
                          </p>
                          @if (issue.expected !== undefined && issue.actual !== undefined) {
                            <div class="mt-1 text-xs" [class]="getIssueTextColor(issue.severity)">
                              Expected: {{ formatNumber(issue.expected) }} | Actual: {{ formatNumber(issue.actual) }}
                            </div>
                          }
                        </div>
                        @if (issue.autoFixable) {
                          <button class="px-3 py-1 bg-white text-sm rounded hover:bg-gray-50">
                            Fix
                          </button>
                        }
                      </div>
              ...
              <div class="mb-6 border-b border-gray-200">
                <nav class="flex space-x-8">
                  <button
                    ...
                    [class]="activeTab() === 'insights' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                  >
                  ...
                  <button
                    ...
                    class="py-2 px-1 border-b-2 font-medium text-sm transition-colors"
                    [class]="activeTab() === 'ratios' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                  >
                    <div class="flex items-center space-x-2">
              ...
              <div>
                @if (activeTab() === 'insights') {
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @for (insight of analysisReport()!.insights; track insight.title) {
                      <div class="p-4 border rounded-lg" [ngClass]="getInsightBorderColor(insight.type)">
                        <div class="flex items-start justify-between mb-2">
                          ...
                          <span class="px-2 py-1 rounded text-xs font-semibold" [ngClass]="getImpactBadgeColor(insight.impact)">
                            {{ insight.impact }}
                          </span>
                        </div>
                  ...
                  @if (analysisReport()!.insights.length === 0) {
                    <div class="text-center py-12">
                      <lucide-icon [name]="LightbulbIcon" [size]="48" class="mx-auto text-gray-300 mb-4" />
                ...
                @if (activeTab() === 'ratios') {
                  ...
                  @for (category of getUniqueCategories(); track category) {
                    <div class="mb-6">
                      ...
                      <div class="space-y-3">
                        @for (ratio of getRatiosByCategory(category); track ratio.name) {
                          <div class="p-4 bg-gray-50 rounded-lg">
                            <div class="flex items-start justify-between mb-2">
                              <div class="flex-1">
                                <h5 class="font-medium text-gray-900">{{ ratio.name }}</h5>
                                <p class="text-xs text-gray-500 mt-1">{{ ratio.formula }}</p>
                              </div>
                            ...
                            <p class="text-sm text-gray-600 mt-2">{{ ratio.interpretation }}</p>
                          </div>
                        }
                      </div>
                    </div>
                  ...
                  @if (analysisReport()!.calculatedRatios.length === 0) {
                    <div class="text-center py-12">
                      <lucide-icon [name]="BarChart3Icon" [size]="48" class="mx-auto text-gray-300 mb-4" />
                      <p class="text-gray-500">No ratios could be calculated from available data</p>
                    </div>
                  }
                }
              </div>
              ...
              <!-- Loading or No Data -->
              <div class="text-center py-12">
                <lucide-icon [name]="ActivityIcon" [size]="48" class="mx-auto text-gray-300 mb-4" />
                <p class="text-gray-500">No financial data available for analysis</p>
              </div>
            }
          </div>
          ...
          <div class="border-t border-gray-200 p-4 bg-gray-50">
            <div class="flex items-center justify-between">
              <p class="text-xs text-gray-500">
                Analysis generated on {{ analysisReport()?.analysisDate | date:'medium' }}
              </p>
...
export class FinancialAnalysisModalComponent {
  ...
  formatRatioValue(ratio: CalculatedRatio): string {
    if (ratio.category === 'profitability' || ratio.category === 'growth' || ratio.category === 'efficiency') {

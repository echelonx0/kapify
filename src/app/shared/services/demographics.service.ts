 ...
 * DemographicsService
 * - Manages demographic data collection for funding requests
 * - Uses DemographicsConfigService for field definitions
 * - Validates data based on remote config
 * - Tracks completion status
 * - Syncs with funding_application_cover_information table
 */
@Injectable({
...
export class DemographicsService implements OnDestroy {
  ...
  // ===== CONFIG ACCESS =====
  readonly config = this.configService.categories;

  constructor() {
    // Config loads automatically in DemographicsConfigService
  }
  ...
  /**
   * Validate demographic data against remote config
   */
  validateDemographics(data: DemographicsData): DemographicsValidationResult {
    const errors: FieldValidationError[] = [];
  ...
  getCategory(categoryId: string): DemographicCategory | undefined {
    return this.configService.getCategory(categoryId);
  }
  ...
  private async ensureConfigLoaded(): Promise<void> {
    ...
    if (this.configService.getCategories().length > 0) {
      return;
    }

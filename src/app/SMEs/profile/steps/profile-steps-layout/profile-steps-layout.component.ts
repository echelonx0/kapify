...
overallProgress = computed(() => {
  ...
  const percentage = this.utilityService.calculateCompletionPercentage(completedSteps, totalSteps);
  console.log('Progress:', { totalSteps, completedSteps, percentage }); // DEBUG
  return percentage;
});
...
ngOnInit() {
  ...
  debugSteps() {
  console.table(this.profileService.steps.map(step => ({
    id: step.id,
    title: step.title,
  ...
  tSectionDataFn = () => (stepId: string) => this.getSectionData(stepId);

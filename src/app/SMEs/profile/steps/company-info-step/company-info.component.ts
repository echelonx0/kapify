...
export class CompanyInfoComponent implements OnInit, OnDestroy {
  ...
  // 🔥 FIX: Track if initial data load is complete
  // This prevents re-loading data when service changes
  ...
  constructor() {
    ...
    // 🔥 FIX: Load data ONLY ONCE when service finishes loading
    // Do NOT reload on every service data change
    effect(() => {
      const isLoading = this.fundingApplicationService.loading();
      const hasLoaded = this.initialLoadComplete();

      // Only load when service finishes AND we haven't loaded yet
    ...
    // 🔥 FIX: Only sync shareholders on explicit changes, not on every service update
    // This prevents form repopulation from overwriting user changes
    effect(() => {
      const serviceData = this.fundingApplicationService.data().companyInfo;

      // Only update shareholders if:
      // 1. Initial load is complete (we have a form)
      // 2. Service data has ownership info
  ...
  private loadExistingData() {
    ...
    if (existingData) {
      ...
      // This ensures form.dirty starts as false (no unsaved changes)
      this.adminForm.markAsPristine();

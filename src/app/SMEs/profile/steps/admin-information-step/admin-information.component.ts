...
export class AdminInformationComponent implements OnInit, OnDestroy {
  ...
  private populateFormsFromData(data: CompanyInformation) {
    this.adminForm.patchValue({
      ...
      lastName:
        data.contactPerson?.fullName?.split(' ').slice(1).join(' ') || '',
      ...
      taxCompliance:
        data.taxComplianceStatus === 'compliant' ? 'compliant' : 'outstanding',

      // Business Stage & Legal Fields
  ...
  private buildCompanyInfoData(): CompanyInformation {
    ...
    // Build address string with optional line 2
    const streetAddress = formValue.addressLine2

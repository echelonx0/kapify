    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
      ...
      <div class="divide-y divide-slate-200">
        @for (row of rows; track $index) {
          <div class="px-6 py-4 hover:bg-slate-50/50 transition-colors duration-200">
            <div class="grid gap-4" [style.grid-template-columns]="gridTemplate">
              <!-- Label column -->
              <div class="flex items-center">
                <div class="skeleton-box h-4 bg-slate-200/70 rounded" 
                     [style.width]="getRandomWidth(120, 200) + 'px'"></div>
              </div>
...
export class FinancialTableSkeletonComponent {
  ...
  /**
   * Generate random width for variety in skeleton
   */
  getRandomWidth(min: number, max: number): number {

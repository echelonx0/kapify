...
export class FundingProfileSetupService implements OnDestroy {
  ...
  /**
   * Get user's active organization with fallback to localStorage
   */
  private async getUserOrganization(userId: string): Promise<string | null> {
    try {
      console.log('🔍 Attempting to fetch organization for user:', userId);
      ...
      if (error) {
        ...
        return this.getOrganizationFromLocalStorage();
      }

      const orgId = data?.organization_id || null;
      if (orgId) {
        console.log('✅ Organization fetched from database:', orgId);
        return orgId;
      }

      return this.getOrganizationFromLocalStorage();
    } catch (error) {
      console.warn('⚠️ Failed to get user organization:', error);
      return this.getOrganizationFromLocalStorage();
    }
  }

  /**
   * Fallback: Get organization from localStorage if available
   */
  private getOrganizationFromLocalStorage(): string | null {
    try {
      const saved = localStorage.getItem(AUTO_SAVE_CONFIG.localStorageKey);
      if (saved) {
      ...
      console.warn('Could not retrieve org from localStorage');
    }
    return null;
  }
  ...
  setCurrentStep(stepId: string) {
    if (this.isValidStep(stepId)) {
      ...
      // Ensure org context is refreshed when changing steps
      const user = this.authService.user();
      if (user && !this.currentOrganization()) {
        this.getUserOrganization(user.id).then((orgId) => {
  ...
  async loadSavedApplication(): Promise<void> {
    try {
      ...
      // Step 1: Load localStorage first (instant, works offline)
      console.log('📂 Loading from localStorage...');
      this.loadFromLocalStorage();

      // Step 2: Get organization context (with fallback)
      console.log('🔍 Resolving organization context...');
      const orgId = await this.getUserOrganization(user.id);
      this.setOrganizationContext(orgId);

      // Step 3: If we have org context, try backend sync
      if (orgId) {
        ...
        try {
          ...
          if (savedData && !this.utilityService.isDataEmpty(savedData)) {
            const mergedData = this.utilityService.mergeApplicationData(
              this.applicationData(),
              savedData
            );
          ...
          console.warn(
            '⚠️ Backend sync failed, continuing with local data:',
            backendError
          );
        ...
        console.warn(
          '⚠️ Could not resolve organization. Using local data only.'
        );
      }

      await this.refreshSlug();
    } catch (error) {
      console.error('❌ Error in loadSavedApplication:', error);
      // Graceful degradation - still have local data
    } finally {
      this.isLoading.set(false);
    }
  }
  ...
  private async saveToBackend(exitApp: boolean = false): Promise<boolean> {
    ...
    try {
      ...
      const orgId = this.currentOrganization();
      if (!orgId) {
        console.warn('⚠️ No organization context for backend save');
  ...
  async submitForReview(): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (!orgId) {
        return {
          success: false,
          error: 'No organization context - please save profile first',
        };
      }
  ...
  async refreshSlug(): Promise<string | null> {
    try {
      const orgId = this.currentOrganization();
      if (!orgId) {
        console.warn('⚠️ No organization context for slug');
        return null;
      }
      ...
      if (error) {
        console.info('ℹ️ No slug found yet (profile may not be saved)');
        return null;
      }

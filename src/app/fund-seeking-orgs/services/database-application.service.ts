...
export class DatabaseApplicationService {
   ...
   * @param applicationData - Must include funderId (organization_id from opportunity)
   */
  createApplication(applicationData: {
    title: string;
    description?: string;
    opportunityId?: string;
    funderId: string; // REQUIRED: Funder organization ID from opportunity
    formData?: any;
    documents?: any;
    fundingRequest: any;
  }): Observable<Application> {
    if (!applicationData.fundingRequest) {
      const error = new Error(
        'fundingRequest is required when creating an application'
      );
      this.error.set(error.message);
      return throwError(() => error);
    }

    // Validate funderId is provided
    if (!applicationData.funderId || applicationData.funderId.trim() === '') {
      const error = new Error(
        'funderId is required. Must pass opportunity.organizationId'
      );
      this.error.set(error.message);
      return throwError(() => error);
    }
  ...
  private async insertApplication(applicationData: {
    ...
    try {
      ...
      const dbApplication: Partial<DatabaseApplication> = {
        ...
        funding_request: applicationData.fundingRequest,
      };
      ...
      console.log(
        '✓ Application created with funder_id:',
        applicationData.funderId
      );
  ...
  private async updateApplicationInDatabase(
    ...
    try {
      ...
      // ❌ Cannot remove funding_request
      if (updates.fundingRequest === null) {
        throw new Error('fundingRequest cannot be null');
      }

      // ❌ Cannot submit without funding_request
      if (isSubmission && existing.funding_request == null) {
        throw new Error('Cannot submit application without funding_request');
      }

      // ❌ Cannot modify after submission
      if (
        existing.status === 'submitted' &&
        updates.fundingRequest !== undefined
      ) {
        throw new Error('Cannot modify fundingRequest after submission');
      }
      ...
      if (updates.fundingRequest !== undefined) {
        dbUpdates.funding_request = updates.fundingRequest;
      }
  ...
  private transformDatabaseToLocal(dbApp: any): Application {
    ...
    return {
      ...
      funderOrganizationId:
        dbApp.funder_id || opportunity?.organization_id || '',
      fundId: '',
      ...
      complianceChecks: [],
      auditTrail: [],
      fundingRequest: dbApp.funding_request,
